 ...
 * `Access-Control-Allow-Origin: *`, so this runs entirely on-device.
 */
...
interface WorkSearchResponse {
  docs?: { ia?: string[]; ebook_access?: string }[];
}

interface EditionsResponse {
  entries?: { ocaid?: string }[];
}
...
function describeSource(ocaid: string): BookTextSource {
  const provider = /gutenberg|gut$/i.test(ocaid)
    ? "Project Gutenberg"
    : "Internet Archive";
  return { ocaid, provider, url: ARCHIVE.details(ocaid) };
}
