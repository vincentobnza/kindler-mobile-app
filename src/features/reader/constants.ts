 ...
 *
 * Both endpoints send `Access-Control-Allow-Origin: *` and are plain HTTPS, so
 * the app fetches them directly — no backend or proxy required.
 */
...
export const ARCHIVE = {
  ...
  /** Canonical details page for an item. */
  details: (ocaid: string): string =>
    `${ARCHIVE_BASE_URL}/details/${encodeURIComponent(ocaid)}`,
} as const;
